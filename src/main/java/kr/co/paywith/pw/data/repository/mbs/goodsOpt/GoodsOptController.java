...
public class GoodsOptController extends CommonController {
  ...
  public ResponseEntity createGoodsOpt(
    ...
    // 현재 로그인 유저 설정
    if (currentUser != null) {
      goodsOpt.setCreateBy(currentUser.getAccountId());
      goodsOpt.setUpdateBy(currentUser.getAccountId());
    }

    // 레코드 등록
    GoodsOpt newGoodsOpt = goodsOptService.create(goodsOpt);

    ControllerLinkBuilder selfLinkBuilder = linkTo(GoodsOptController.class)
        .slash(newGoodsOpt.getId());
...
//		  User princpal = (User) authentication.getPrincipal();
  ...
  public ResponseEntity putGoodsOpt(@PathVariable Integer id,
    ...
    // 현재 로그인 유저 설정
    if (currentUser != null) {
      existGoodsOpt.setUpdateBy(currentUser.getAccountId());
    }
