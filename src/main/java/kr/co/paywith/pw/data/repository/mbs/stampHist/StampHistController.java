...
public class StampHistController extends CommonController {
  ...
  public ResponseEntity createStampHist(
    ...
    ControllerLinkBuilder selfLinkBuilder = linkTo(StampHistController.class)
        .slash(newStampHist.getId());
    ...
    stampHistResource
        .add(new Link("/docs/index.html#resources-stampHist-create").withRel("profile"));
...
//	 }
  ...
  public ResponseEntity removeStampHist(@PathVariable Integer id,
      @RequestBody(required = false) StampHistDeleteDto stampHistDeleteDto,
      Errors errors,
      @CurrentUser Account currentUser) {
    // 입력체크
    if (errors.hasErrors()) {
