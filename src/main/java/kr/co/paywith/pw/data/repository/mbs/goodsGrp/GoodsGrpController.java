...
import kr.co.paywith.pw.data.repository.mbs.goodsGrp.GoodsGrpRepository;
import kr.co.paywith.pw.data.repository.mbs.goodsGrp.GoodsGrpResource;
import kr.co.paywith.pw.data.repository.mbs.goodsGrp.GoodsGrpService;
import kr.co.paywith.pw.data.repository.mbs.goodsGrp.GoodsGrpUpdateDto;
import kr.co.paywith.pw.data.repository.mbs.goodsGrp.GoodsGrpValidator;
import kr.co.paywith.pw.data.repository.mbs.goodsGrp.QGoodsGrp;
...
public class GoodsGrpController extends CommonController {
  ...
  public ResponseEntity createGoodsGrp(
    ...
    // 현재 로그인 유저 설정
    if (currentUser != null) {
      goodsGrp.setCreateBy(currentUser.getAccountId());
      goodsGrp.setUpdateBy(currentUser.getAccountId());
    }

    // 레코드 등록
    GoodsGrp newGoodsGrp = goodsGrpService.create(goodsGrp);

    ControllerLinkBuilder selfLinkBuilder = linkTo(GoodsGrpController.class)
        .slash(newGoodsGrp.getId());
...
//        GoodsGrpr princpal = (GoodsGrpr) authentication.getPrincipal();
  ...
  public ResponseEntity putGoodsGrp(@PathVariable Integer id,
    ...
    // 현재 로그인 유저 설정
    if (currentUser != null) {
      existGoodsGrp.setUpdateBy(currentUser.getAccountId());
    }
