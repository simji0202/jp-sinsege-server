...
public enum ChrgSetleMthdCd implements EnumMapperType {
  ...
  CPN("쿠폰", Arrays.asList("CPN")),
  MRGD("병합", Arrays.asList("MRGD")),
  ETC("기타", Arrays.asList("ETC")),
  ...
  ChrgSetleMthdCd(String title, List<String> mthdList) {
    this.title = title;
    this.mthdList = mthdList;
  }

  public static ChrgSetleMthdCd findByInsttCd(String insttCd) {
    if (insttCd == null) {
      return EMPTY;
    } else {
      return Arrays.stream(ChrgSetleMthdCd.values())
