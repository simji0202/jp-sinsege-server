...
public class CpnIssuController extends CommonController {
   ...
   * 발급 규칙에 대한 정의 예정
   *
   * 1. 쿠폰의 발급은 반드시 cpnIssu 을 통해서 <---   o. x 2. 쿠폰 발급과 동시에 쿠폰 번호 부여 여부  <---   o. x
   */
  ...
  public ResponseEntity createCpnIssu(
    ...
    // 현재 로그인 유저 설정
    if (currentUser != null) {
      cpnIssu.setCreateBy(currentUser.getAccountId());
      cpnIssu.setUpdateBy(currentUser.getAccountId());
    }

    // 레코드 등록
    CpnIssu newCpnIssu = cpnIssuService.create(cpnIssu, currentUser);

    ControllerLinkBuilder selfLinkBuilder = linkTo(CpnIssuController.class)
        .slash(newCpnIssu.getId());
...
//        User princpal = (User) authentication.getPrincipal();
  ...
  public ResponseEntity putCpnIssu(@PathVariable Integer id,
    ...
    // 현재 로그인 유저 설정
    if (currentUser != null) {
      existCpnIssu.setUpdateBy(currentUser.getAccountId());
    }
