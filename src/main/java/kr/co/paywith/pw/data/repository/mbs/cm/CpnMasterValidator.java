...
public class CpnMasterValidator {
  ...
  public void validate(CpnMasterDto cpnMasterDto, Errors errors) {
    ...
    // 쿠폰 마스터 종류 (필수 체크)
    ValidatorUtils.checkObjectNull(cpnMasterDto.getCpnMasterTypeCd(), "쿠폰 마스터 종류", errors);
...
//
    ...
    if (CpnMasterTypeCd.GOODS.equals(cpnMasterDto.getCpnMasterTypeCd())) {
      ValidatorUtils.checkObjectNull(cpnMasterDto.getGoodsId(), "쿠폰 적용 상품", errors);
    }

    // 금액 쿠폰 경우  할인금액 확인
    if (CpnMasterTypeCd.AMT.equals(cpnMasterDto.getCpnMasterTypeCd())) {
      ValidatorUtils.checkInteger(cpnMasterDto.getCpnAmt(), "할인금액", errors, false, 1, 99999999);
    }

    // 할인 쿠폰일 경우 할인율 확인
    if (CpnMasterTypeCd.RATIO.equals(cpnMasterDto.getCpnMasterTypeCd())) {
