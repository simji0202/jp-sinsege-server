...
//					 .build();
	 ...
	 public void getBillings() throws Exception {
		  // Given
		  //  IntStream.range(0, 30).forEach(this::generateBilling);

		  // When & Then
		  this.mockMvc.perform(get("/api/billing")
					 .header(HttpHeaders.AUTHORIZATION, getBearerToken(true))
					 .header("Origin", "*")
					 .param("page", "0")
					 .param("size", "10")
					 ...
					 .andExpect(jsonPath("_embedded.billingList[0]._links.self").exists())
					 .andExpect(jsonPath("_links.self").exists())
					 .andExpect(jsonPath("_links.profile").exists())
					 .andDo(document("query-billings"))
		  ;
	 }
	 ...
	 public void getBilling() throws Exception {
		  // Given
		  //  Billing billing = this.generateBilling(100);

		  // When & Then
		  this.mockMvc.perform(get("/api/billing/{id}", 1)
					 .header("Origin", "*")
					 .header(HttpHeaders.AUTHORIZATION, getBearerToken(true))
		  )
					 .andDo(print())
	 ...
	 public void updateBilling() throws Exception {
		  ...
		  this.mockMvc.perform(put("/api/billing/{id}", billing.getId())
					 ...
					 .contentType(MediaType.APPLICATION_JSON_UTF8)
					 .content(this.objectMapper.writeValueAsString(billing)))
					 .andDo(print())
					 .andExpect(status().isOk())
					 .andExpect(jsonPath("_links.self").exists())
					 .andDo(document("update-billing"))
		  ;
	 }
