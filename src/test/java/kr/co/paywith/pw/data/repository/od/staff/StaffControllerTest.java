...
//					 .build();
	 ...
	 public void getStaffs() throws Exception {
		  // Given
		  //  IntStream.range(0, 30).forEach(this::generateStaff);

		  // When & Then
		  this.mockMvc.perform(get("/api/staff")
					 .header(HttpHeaders.AUTHORIZATION, getBearerToken(true))
					 .header("Origin", "*")
					 .param("page", "0")
					 .param("size", "10")
					 ...
					 .andExpect(jsonPath("_embedded.staffList[0]._links.self").exists())
					 .andExpect(jsonPath("_links.self").exists())
					 .andExpect(jsonPath("_links.profile").exists())
					 .andDo(document("query-staffs"))
		  ;
	 }
	 ...
	 public void getStaff() throws Exception {
		  // Given
		  //  Staff staff = this.generateStaff(100);

		  // When & Then
		  this.mockMvc.perform(get("/api/staff/{id}", 1)
					 .header("Origin", "*")
					 .header(HttpHeaders.AUTHORIZATION, getBearerToken(true))
		  )
					 .andDo(print())
	 ...
	 public void updateStaff() throws Exception {
		  ...
		  // When & Then
		  this.mockMvc.perform(put("/api/staff/{id}", staff.getId())
					 .header(HttpHeaders.AUTHORIZATION, getBearerToken(true))
					 .header("Origin", "*")

					 .contentType(MediaType.APPLICATION_JSON_UTF8)
					 .content(this.objectMapper.writeValueAsString(staff)))
					 .andDo(print())
					 .andExpect(status().isOk())
					 .andExpect(jsonPath("_links.self").exists())
					 .andDo(document("update-staff"))
		  ;
	 }
